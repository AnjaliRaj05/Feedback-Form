...
interface Feedback {
  ...
  formData: {
    'full-name': string;
    email: string;
    'platform-satisfaction': string;
    'usability-satisfaction': string;
    'feature-suggestions': string;
    'nps-score': string;
  };
  createdAt: string;
}
...
const FeedbackData = () => {
  ...
  const fetchFeedbackData = async () => {
    try {
      const response = await axios.get<Feedback[]>('https://feedback-backend-tog9.onrender.com/api/v1/feedback-data');
